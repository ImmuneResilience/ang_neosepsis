---
title: "Kollman neonatal mouse sepsis project: liver transcriptome"
output: 
  html_document:
    theme: cerulean
...
## Background
This script performs differential expression analysis for the mouse liver transcriptome.
...
samples_all <- read.csv("../../../Rdata/Mouse/rnaseq/metadata/samples_liver_fastq.csv", header=TRUE) %>% 
...
write.csv(samples_new, "../../../results/samples_liver_libsizes.csv", row.names = FALSE)
```
...
saveRDS(object = vsd, file = "../../../results/liver_vst.rds")
```
...
```{r}
group.contrast(dds, "score", "CS_Survive", "Control", "liver")
```

#### Die versus control
```{r}
group.contrast(dds, "score", "CS_Die", "Control", "liver")
```

#### Die versus survive
```{r}
group.contrast(dds, "score", "CS_Die", "CS_Survive", "liver")
```
