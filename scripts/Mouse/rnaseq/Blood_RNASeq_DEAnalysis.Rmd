---
title: "Kollman neonatal mouse sepsis project: blood transcriptome"
output: 
  html_document:
    theme: cerulean
...
## Background
This script performs differential expression analysis for the mouse blood transcriptome.
...
samples_all <- read.csv("../../../Rdata/Mouse/rnaseq/metadata/samples_blood_fastq.csv", header=TRUE)

# read in the metadata
metadata <- read.csv("../../../Rdata/Mouse/rnaseq/metadata/rnaseq.metadata.csv", header = TRUE)

metadata_subset <- metadata %>% 
  dplyr::select(rnaID, classification, classification_live, chal.status) 

# Remove samples from the analyses
samples <- samples_all %>% 
  filter(LibraryName != "mS05") %>%  # Female, challenged, sacrificed at 18h rather than 24h
  filter(LibraryName != "mS09") %>%  # Male, non-challenged, high CFU in blood, spleen, obviously sick
  filter(LibraryName != "mS14") %>%  # Female, challenged, gaining weight (belong to a different class)
  filter(LibraryName != "mS21") %>%  # Male, non-challenged, but losing weight, probably sick
  filter(LibraryName != "mS22") %>%  # Male, challenged, gaining weight (belong to a different class)
  filter(LibraryName != "mS11") %>%  # female, non-challenged, has CFU in liver/spleen
  left_join(., metadata_subset, by = c("LibraryName" = "rnaID")) 
...
good_libsize <- libsize.post %>% 
  filter(libsize_noglobin > 1000000) # library sizes > 1M reads, only 24 samples left
...
write.csv(samples_new, "../../../results/samples_blood_libsizes.csv", row.names = FALSE)
```
...
saveRDS(object = vsd, file = "../../../results/blood_vst.rds")
```
...
```{r}
group.contrast(dds, "score", "CS_Survive", "Control", "blood")
```

#### Die versus control
```{r}
group.contrast(dds, "score", "CS_Die", "Control", "blood")
```

#### Die versus survive
```{r}
group.contrast(dds, "score", "CS_Die", "CS_Survive", "blood")
```
