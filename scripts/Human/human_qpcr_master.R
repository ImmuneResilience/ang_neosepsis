...
# load data
dat <- read.csv("Rdata/Human/human_qpcr_master.csv")
...
ggplot(dat[dat$gene != "Cyp1b1",], aes(Type, logfc, fill = Type)) +
  ...
  stat_compare_means(size = 3, comparisons = list(c("Control", "Sepsis")), label = "p.signif") +
...
ggplot(dat, aes(Type, logfc)) +
  geom_point(position = position_jitter(width = 0.05), shape = 21, size = 0.8) +
  geom_boxplot(outlier.shape = NA, alpha = 0.5, size = 0.75) +
  stat_compare_means(comparisons = list(c("Control", "Sepsis")), label = "p.signif") +
  facet_wrap(~gene, scales = "free", ncol = 4) +
  theme_classic()+ 
  box.theme

# save
ggsave(paste0(output.dir, "human_qpcr_control_case_alltargets.pdf"),
       device = "pdf",
       dpi = 300,
       width = 5.8, height = 3.5)
